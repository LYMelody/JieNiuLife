...
- (void)setTag:(NSInteger)tag {
    ...
    _tag = tag;
    if (tag == 101) {
        _segmentControl.frame = CGRectMake(0, [self AutoHeight:293], [UIScreen mainScreen].bounds.size.width, [self AutoHeight:38]);
        _scrollView.frame = CGRectMake(0, CGRectGetMaxY(_segmentControl.frame), [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height - CGRectGetMaxY(_segmentControl.frame));
    }
